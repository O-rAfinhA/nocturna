...
PRIVACY = {
    "pt": {
        "title": "Privacidade", "back": "Voltar ao atlas", "status": "Responsável pelo Nocturna: Rafael Correia.",
        "intro": "Esta página descreve o funcionamento atual do Nocturna. Visitantes não precisam de conta e o site não exibe anúncios.",
        "h1": "Sua localização", "p1": "O navegador só pede acesso à localização quando você clica em “Minha localização”. As coordenadas são usadas na memória desta página para centralizar o globo e ordenar as histórias publicadas por distância. O código do site não as grava nem as coloca no endereço da página. Você pode negar a permissão e continuar usando a busca ou o globo. O próprio navegador permite revisar a permissão depois.",
        ...
        "h5": "Medição de visitas e publicidade", "p5": "Se você permitir no aviso de privacidade, o site usa Vercel Web Analytics para contar visitas de forma agregada, sem cookies de análise e sem identificar visitantes individualmente. A medição pode incluir a página acessada, origem da visita, país aproximado e dados técnicos do navegador; esses dados são processados pela Vercel. Você pode mudar sua escolha em ‘Preferências de privacidade’ no rodapé. A preferência é guardada no armazenamento local do navegador. Google Analytics e Google AdSense não estão ativos.",
        "h6": "Contato e seus direitos", "p6": "Para solicitar informações sobre o tratamento de seus dados ou pedir a correção ou exclusão de um comentário, escreva para",
    },
    "en": {
        "title": "Privacy", "back": "Back to the atlas", "status": "Nocturna is operated by Rafael Correia.",
        "intro": "This page describes the current operation of Nocturna. Visitors do not need an account and the site does not display ads.",
        "h1": "Your location", "p1": "The browser requests location access only when you click “My location”. Coordinates are used in this page's memory to center the globe and sort published stories by distance. The site code does not save them or put them in the page URL. You can refuse permission and still use search or the globe. You can review this permission in your browser later.",
        ...
        "h5": "Visit measurement and ads", "p5": "If you allow it in the privacy notice, the site uses Vercel Web Analytics to count visits in aggregate, without analytics cookies or individually identifying visitors. Measurement may include the page visited, referral source, approximate country and browser technical data; Vercel processes this data. You can change your choice using ‘Privacy preferences’ in the footer. The preference is saved in your browser's local storage. Google Analytics and Google AdSense are not active.",
        "h6": "Contact and your rights", "p6": "To ask how your data is handled or request correction or deletion of a comment, write to",
    },
    "es": {
        "title": "Privacidad", "back": "Volver al atlas", "status": "Responsable de Nocturna: Rafael Correia.",
        "intro": "Esta página describe el funcionamiento actual de Nocturna. Los visitantes no necesitan una cuenta y el sitio no muestra anuncios.",
        "h1": "Tu ubicación", "p1": "El navegador solo solicita acceso a la ubicación cuando pulsas “Mi ubicación”. Las coordenadas se usan en la memoria de esta página para centrar el globo y ordenar las historias publicadas por distancia. El código del sitio no las guarda ni las añade a la URL. Puedes denegar el permiso y seguir usando la búsqueda o el globo. Después puedes revisar el permiso en el navegador.",
        ...
        "h5": "Medición de visitas y publicidad", "p5": "Si lo permites en el aviso de privacidad, el sitio usa Vercel Web Analytics para contar visitas de forma agregada, sin cookies de análisis ni identificación individual de visitantes. La medición puede incluir la página visitada, el origen de la visita, el país aproximado y datos técnicos del navegador; Vercel procesa estos datos. Puedes cambiar tu elección en ‘Preferencias de privacidad’ en el pie de página. La preferencia se guarda en el almacenamiento local del navegador. Google Analytics y Google AdSense no están activos.",
        "h6": "Contacto y tus derechos", "p6": "Para consultar cómo se tratan tus datos o solicitar la corrección o eliminación de un comentario, escribe a",
    },
}
LOCAL_PRIVACY_ANALYTICS = {
    "pt": "Esta versão local não inclui medição de visitas nem anúncios. Google Analytics e Google AdSense não estão ativos.",
    "en": "This local version does not include visit measurement or ads. Google Analytics and Google AdSense are not active.",
    "es": "Esta versión local no incluye medición de visitas ni anuncios. Google Analytics y Google AdSense no están activos.",
}
...
for lang in ("pt", "en", "es"):
    ...
    page = page.replace('src="./privacy.js"', 'src="../privacy.js"' + (' data-analytics="vercel"' if not STATIC_CATALOG else ''))
    ...
    copy = PRIVACY[lang] if not STATIC_CATALOG else {**PRIVACY[lang], "p5": LOCAL_PRIVACY_ANALYTICS[lang]}
    locale = dict(pt="pt-BR", en="en", es="es")[lang]
    privacy_meta = '<meta name="robots" content="noindex,follow">'
    if not STATIC_CATALOG:
        privacy_url = f"{SITE_URL}/{lang}/privacidade.html"
        privacy_alternates = "".join(
            f'<link rel="alternate" hreflang="{code_locale}" href="{SITE_URL}/{code}/privacidade.html">'
            for code, code_locale in (("pt", "pt-BR"), ("en", "en"), ("es", "es"))
        )
        privacy_meta = (f'<meta name="robots" content="index,follow">'
                        f'<link rel="canonical" href="{privacy_url}">{privacy_alternates}')
    contact_email = "aibusinessbr@gmail.com"
    contact_link = f'<a href="mailto:{contact_email}">{contact_email}</a>'
    privacy_script = PRIVACY_SCRIPT if not STATIC_CATALOG else '<script defer src="/privacy.js"></script>'
    legal = f'''<!doctype html>
<html lang="{locale}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">{privacy_meta}<meta name="theme-color" content="#080d14"><title>{escape(copy['title'])} — Nocturna</title><link rel="stylesheet" href="../styles.css">{privacy_script}</head>
<body><div class="shell"><header class="masthead"><a class="brand" href="/{lang}/"><span class="brand-mark" aria-hidden="true">✦</span><span>NOCTURNA</span></a><label class="language"><span class="sr-only">{escape(copy['title'])}</span><select id="legal-language" aria-label="{dict(pt='Idioma',en='Language',es='Idioma')[lang]}"><option value="pt" {'selected' if lang=='pt' else ''}>PT</option><option value="en" {'selected' if lang=='en' else ''}>EN</option><option value="es" {'selected' if lang=='es' else ''}>ES</option></select></label></header><main class="legal"><a class="back" href="/{lang}/">← {escape(copy['back'])}</a><h1>{escape(copy['title'])}</h1><p class="draft">{escape(copy['status'])}</p><p>{escape(copy['intro'])}</p><h2>{escape(copy['h1'])}</h2><p>{escape(copy['p1'])}</p><h2>{escape(copy['h2'])}</h2><p>{escape(copy['p2'])}</p><h2>{escape(copy['h3'])}</h2><p>{escape(copy['p3'])}</p><h2>{escape(copy['h4'])}</h2><p>{escape(copy['p4'])}</p><h2>{escape(copy['h5'])}</h2><p>{escape(copy['p5'])}</p><h2>{escape(copy['h6'])}</h2><p>{escape(copy['p6'])} {contact_link}.</p></main><footer><a class="footer-brand brand" href="/{lang}/" aria-label="Nocturna"><span class="brand-mark" aria-hidden="true">✦</span><span>NOCTURNA</span></a></footer></div><script>document.getElementById('legal-language').addEventListener('change',function(){{location.href='/'+this.value+'/privacidade.html'}})</script></body></html>'''
