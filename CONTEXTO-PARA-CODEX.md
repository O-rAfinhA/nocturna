...
5. O aviso de privacidade em PT/EN/ES controla o carregamento do Vercel Web Analytics: a medição agregada sem cookies só inicia após escolha afirmativa, salva no armazenamento local e revisável pelo rodapé. O aviso atual não é uma CMP para publicidade. Google Analytics aguarda um ID de medição. O `dist/ads.txt` contém a linha oficial obtida da conta AdSense do usuário; a propriedade foi verificada por esse arquivo e a revisão do site foi solicitada. Anúncios ainda não estão ativos. O responsável e o contato público já constam das páginas de privacidade. A propriedade HTTPS do Search Console usa metatag no código e recebeu o sitemap; a propriedade de domínio completo ainda exige TXT na Hostinger. Antes de ativar anúncios, revisar mapa, busca, privacidade e CMP.
